const ENUSTermsTranslation = {
    "title": "Terms of Service",
    "content":`
    1. Acceptance of Terms of Use
    By agreeing to the Terms of Use, you agree to use the Service. Your continued use of the Service constitutes your agreement to these Terms of Use.
    
    2.Changes to the Terms of Use
    The Service may modify the Terms of Service from time to time as necessary. Modified Terms of Service shall become effective at the time of publication on the Service.

    3. Contents of the Service
    The Service provides the following services to users.
    3.1. Playing music games 
    3.2. Interaction with other companies using online functions
    3.3. Creation and sharing of original contents

    4. User's Responsibility
    Users shall not engage in any of the following acts when using the Service.
    4.1. Slandering, selling, lending, transferring, leasing, or contracting with others using the online functions of the Service.
    4.2. Any illegal acts against others using the Service.
    4.3. Unauthorized diversion of the contents of the Service (whether commercial or non-commercial).

    5. Complaints and consultation regarding the Service
    Please use the in-game report function for complaints, consultations, and reports regarding the Service.

    6.Copyrights related to the Service
    Copyrights to the Service belong to the copyright holders of the Service for all contents related to the Service.
    6.1. Games in general: Mksk
    6.2. Original music of the Service: Raetan, Goners
    6.3. Original images of the Service: Mksk,Raetan,Goners

    7.Suspension of Service Use
    Users may stop using the Service or delete their accounts at any time.
    7.1. Account Suspension
    The Service reserves the right to suspend a user's account if the user is found to be in violation of the Terms of Service.

    8. Suspension of Service Provision
    When the Service is to be suspended or terminated, the Service will notify the user of the suspension.
    `,
    "button":{
        "accept": "Accept",
        "decline": "Decline"
    },
    "termsMessage":{
        "decline":"You didn't agreed to the Terms of Use. If you don't agree to the Terms of Use, you may not use this service."
    }
}

export default ENUSTermsTranslation;