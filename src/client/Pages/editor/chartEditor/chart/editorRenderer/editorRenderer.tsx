...
const ChartEditorRenderer = React.memo((props:{current: chartEditorChart, setCurrent: React.Dispatch<React.SetStateAction<chartEditorChart>>}) => {
    ...
    const [noteTypeIntegrity, setNoteTypeIntegrity] = React.useState(false);
    ...
    function updateCursor(e: React.MouseEvent<HTMLDivElement, MouseEvent>) {
        ...
        const y = (e.pageY - (boundingBox.top+32)) / (boundingBox.height - 36) * 100
        const nearestY = getNearest(y, horizonalAnchor.map(a => a.position));
        if (nearestY != 0) setCurosrPosY(nearestY);
        updateCursorNoteIcon(nearestY||cursorPosY);
    }
    function updateCursorNoteIcon(nearestY:number){
        const type = selectedMode.type;
        const lineType = horizonalAnchor.find(h=>h.position==nearestY);
        setNoteTypeIntegrity(type == lineType?.type);
    }
        ...
        <div className={style.editorRenderer}>
            ...
            <div className={style.canvasContainer} ref={canvasContainerRef} onMouseMove={updateCursor}>
                <div className={style.editorCanvas} ref={editorCanvasRef}>
                    ...
                    <div className={style.editorView}>
                        ...
                        <div className={style.cursorObjectContainer}>
                            ...
                            <div className={`${style.noteIcon} ${style[selectedMode.name]} ${!noteTypeIntegrity ? style.inactive : ""}`} style={{ left: `${cursorPosX}px`, top: `${cursorPosY}%` }} />
                        </div>
                    </div>
                </div>
            </div>
