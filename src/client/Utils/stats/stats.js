...
var Stats = function () {
    ...
    return {
        ...
        begin: function () {
            ...
            MusicGameVariables.Performance.delay = ms = time - startTime;
            MusicGameVariables.Performance.minDelay = msMin = Math.min(msMin, ms);
            MusicGameVariables.Performance.maxDelay = msMax = Math.max(msMax, ms);
            ...
            if (time > prevTime + 1000) {
                ...
                MusicGameVariables.Performance.fps = fps = Math.round((frames * 1000) / (time - prevTime));
                MusicGameVariables.Performance.minFps = fpsMin = Math.min(fpsMin, fps);
                MusicGameVariables.Performance.maxFps = fpsMax = Math.max(fpsMax, fps);
                ...
                if (mem !== undefined) {
                    ...
                    MusicGameVariables.Performance.memory = mem = Math.round(heapSize * 0.000000954);
                    MusicGameVariables.Performance.minMemory = memMin = Math.min(memMin, mem);
                    MusicGameVariables.Performance.maxMemory = memMax = Math.max(memMax, mem);
